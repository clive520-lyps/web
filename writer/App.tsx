...
export default function App() {
  ...
  const handleAutoPlan = async () => {
    ...
    setStatus('planning');
    try {
      const plan = await autoPlanEssay(config.title, config.summary);
  ...
  const handleGenerateEssay = async () => {
    ...
    setAudioBlob(null);
    try {
      const content = await generateEssayContent(config);
  ...
  const handleGenerateSpeech = async () => {
    ...
    try {
      const base64Audio = await generateSpeech(essayContent);
