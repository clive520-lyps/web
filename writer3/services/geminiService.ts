...
export const autoPlanEssay = async (title: string, summary: string, apiKey: string): Promise<Partial<EssayConfig>> => {
...
export const generateEssayContent = async (config: EssayConfig, apiKey: string): Promise<string> => {
  if (!apiKey) throw new Error("API Key is missing");
  const ai = new GoogleGenAI({ apiKey });
